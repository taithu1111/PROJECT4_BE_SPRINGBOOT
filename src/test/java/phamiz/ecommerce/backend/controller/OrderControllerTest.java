...
import phamiz.ecommerce.backend.dto.Order.AddressDTO;
import phamiz.ecommerce.backend.dto.Order.OrderDTO;
import phamiz.ecommerce.backend.dto.Order.OrderItemDTO;
...
public class OrderControllerTest {
        ...
        void shouldReturnUsersOrderHistory() throws Exception {
                ...
                order1.setUserId(mockUser.getId());
                order1.setUserEmail(mockUser.getEmail());
                ...
                order2.setUserId(mockUser.getId());
                order2.setUserEmail(mockUser.getEmail());
        ...
        void shouldReturnOrdersSortedByDate() throws Exception {
                ...
                // Setup mock OrderDTOs with different dates
                AddressDTO address = new AddressDTO();
                address.setStreetAddress("789 Oak Ave");
                address.setCity("Chicago");
                address.setZipCode("60601");

                OrderDTO oldOrder = new OrderDTO();
                oldOrder.setId(200L);
                oldOrder.setOrderId("ORDER-OLD");
                oldOrder.setUserId(mockUser.getId());
                oldOrder.setUserEmail(mockUser.getEmail());
                oldOrder.setShippingAddress(address);
                oldOrder.setOrderDate(LocalDateTime.now().minusDays(5));
                oldOrder.setOrderStatus("DELIVERED");
                oldOrder.setTotalPrice(200.0);
                oldOrder.setOrderItems(new ArrayList<>());

                OrderDTO recentOrder = new OrderDTO();
                recentOrder.setId(201L);
                recentOrder.setOrderId("ORDER-RECENT");
                recentOrder.setUserId(mockUser.getId());
                recentOrder.setUserEmail(mockUser.getEmail());
                ...
                recentOrder.setTotalPrice(400.0);
                recentOrder.setOrderItems(new ArrayList<>());

                // Assuming service returns in order (most recent first or oldest first)
                List<OrderDTO> orders = Arrays.asList(recentOrder, oldOrder);
        ...
        void shouldHandleOrderExceptionWhenOrderNotFound() throws Exception {
                ...
                when(orderService.findOrderById(999L))
                                .thenThrow(new phamiz.ecommerce.backend.exception.OrderException(
                                                "order not exist with id: 999"));

                // Perform request
                mockMvc.perform(get("/api/orders/999")
                                .header("Authorization", "Bearer mock-jwt-token")
                                .contentType(MediaType.APPLICATION_JSON))
                                .andExpect(status().isBadRequest()); // OrderException returns BAD_REQUEST via
